...
var roleHarvester = {
    ...
    run: function (creep) {
        ...
        if (creep.memory.working) {
            ...
            else
            {
                ...
                {
                    case 'spawner':
                        var result = creep.transfer(Game.getObjectById(creep.memory.object), RESOURCE_ENERGY);
                        if(result!=OK)
                        {
                            this.getPath(creep);
                        }
    ...
    getPath: function (creep) {
        ...
        var result =helper.getTarget(creep, 'spawner');
        if(result===false) {
            var result =helper.getTarget(creep, 'tower');
            if(result===false) {
